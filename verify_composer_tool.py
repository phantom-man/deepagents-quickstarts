...
from DeepAgents.CommercialAgents.composer_agent.agent import create_composer_agent, generate_music_tool
...
def test_composer():
    ...
    # 1. Create Agent
    try:
        agent = create_composer_agent(model_config={"provider": "Google", "model": "gemini-1.5-flash"}, session_id="test_verify")
        print("✅ Agent created.")
    ...
    # But we can verify if generate_music_tool is what we think it is.
    
    print(f"Tool Name: {generate_music_tool.name}")
    print(f"Tool Description: {generate_music_tool.description}")
