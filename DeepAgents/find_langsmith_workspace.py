...
try:
    ...
    response = requests.get(
        "https://api.smith.langchain.com/workspaces", headers=headers
    )
    ...
    if response.status_code == 200:
        ...
        for w in data:
            print(
                f"   Name: {w.get('display_name', 'Unknown')} | ID: {w.get('id')} | Tenant Handle: {w.get('tenant_handle')}"
            )
    else:
        ...
        resp2 = requests.get(
            "https://api.smith.langchain.com/orgs/current", headers=headers
        )
