...
def inspect_minimax():
    ...
    try:
        ...
        print("Schema Keys:", version.openapi_schema.keys() if hasattr(version, 'openapi_schema') else "No schema attr")
        
        if hasattr(version, 'openapi_schema'):
            inputs = version.openapi_schema.get("components", {}).get("schemas", {}).get("Input", {})
