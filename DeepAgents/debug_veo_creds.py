...
print(f"1. GOOGLE_APPLICATION_CREDENTIALS env var: {env_creds if env_creds else 'Not Set (Correct for ADC)'}")
...
if os.path.exists(ADC_PATH):
    ...
    try:
        with open(ADC_PATH, 'r') as f:
            ...
            print(f"   [INFO] Quota Project in JSON: {data.get('quota_project_id', 'N/A')}")
...
try:
    ...
    if hasattr(credentials, 'info'): # Service Account
        ...
        print(f"   [INFO] User Client ID matches ADC? {'Yes' if data.get('client_id') == credentials.client_id else 'No'}")
