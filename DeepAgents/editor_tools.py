...
@tool
def merge_video_audio(video_paths: List[str], audio_path: str, output_name: str = "final_cut.mp4") -> str:
    """
    ...
    """
    logger.info("✂️ Editor > Merging %d clips with audio %s...", len(video_paths), audio_path)
        ...
        try:
            with open(p, 'rb') as f:
                ...
                try:
                    ...
                    if "[FINAL VIDEO SIMULATION]" in text_preview or "[SIMULATED]" in text_preview or "[AUDIO]" in text_preview or "[VISUAL PANEL]" in text_preview:
                         real_files = False
    ...
    if not real_files:
        logging.info("ℹ️ Simulation Mode triggered by missing MoviePy or detected mock assets.")
        # Mock Merge
        assets = AssetManager()
    ...
    try:
        ...
        final_video.write_videofile(final_path, codec="libx264", audio_codec="aac", fps=24, logger=None)
        logger.info("✅ Render Complete: %s", final_path)
        return final_path

    except Exception as e:
        logger.error("Merge failed with MoviePy: %s. Falling back to Simulation Merge.", e)
        # Fallback to Simulation Merge
        assets = AssetManager()
        content = f"[FINAL VIDEO SIMULATION - FALLBACK]\nReason: MoviePy Failure ({e})\nVideo Sources: {video_paths}\nAudio Source: {audio_path}\n"
        path = assets.save_asset(content, "video", "final_cut_fallback", "Merged Video (Fallback)")
