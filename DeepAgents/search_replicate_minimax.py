    ...
    if hasattr(replicate.models, 'search'):
        ...
        for model in results:
             ...
             if latest:
                 ...
                 print(f"  Schema: {json.dumps(latest.openapi_schema['components']['schemas']['Input'], indent=2) if 'components' in latest.openapi_schema else 'N/A'}")
    else:
        # Fallback: Try specific known owners
        candidates = ["cjwbw/minimax-speech-01", "replicate/minimax", "minimax/speech-01"]
        for c in candidates:
            try:
                m = replicate.models.get(c)
                print(f"FOUND: {m.owner}/{m.name}")
                print(f"  Inputs: {json.dumps(m.latest_version.openapi_schema, indent=2)}")
            except:
                pass
