...
def add_command(prompt: str):
    ...
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('INSERT INTO command_queue (prompt, status, created_at) VALUES (?, ?, ?)', 
...
def pop_command() -> str | None:
    ...
    # Get oldest pending
    cursor.execute('SELECT id, prompt FROM command_queue WHERE status = "pending" ORDER BY created_at ASC LIMIT 1')
    row = cursor.fetchone()
    
    if row:
        cmd_id, prompt = row
        # Mark as processed (or delete) - let's mark as processed to keep history
        cursor.execute('UPDATE command_queue SET status = "processed" WHERE id = ?', (cmd_id,))
        conn.commit()
        conn.close()
        return prompt
...
def clear_queue():
    ...
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('UPDATE command_queue SET status = "cleared" WHERE status = "pending"')
    conn.commit()
    conn.close()
