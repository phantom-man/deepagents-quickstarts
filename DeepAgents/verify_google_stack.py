...
def test_gemini_3_pro():
    ...
    try:
        # Use Client
        client = genai.Client(
            vertexai=True, project=PROJECT_ID, location=LOCATION_GLOBAL
        )
...
def test_lyria_2():
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "Skipping deep protocol implementation for Lyria in this quick probe."
            )
...
def check_quotas():
    ...
    logger.info(
        "To see exact numbers, run: `gcloud services quota list --service=aiplatform.googleapis.com --consumer=projects/YOUR_PROJECT` in terminal."
    )
    ...
    try:
        ...
        if res.returncode == 0:
            ...
            logger.info(
                "Could not auto-retrieve granular quota info via gcloud (Auth/Install issue implies manual check needed)."
            )
    except Exception:
        pass
