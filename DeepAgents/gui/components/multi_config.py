...
def render_multi_config_panel(
    ...
    for idx in range(count):
        with st.expander(
            f"🎬 {agent_type.title()} #{idx + 1} Configuration", expanded=(idx == 0)
        ):
            ...
            ui_generator_indexed = DynamicUIGenerator(
                key_prefix=f"{key_prefix}_multi_{idx}"
            )
            ...
            schema_params = ui_generator_indexed.render_controls(
                ...
                + text_fields,  # Exclude text fields already rendered
                columns=2,
            )
...
def _get_field_info(field_name: str, agent_type: str, model_id: str) -> Dict[str, Any]:
    ...
    defaults = {
        "prompt": {
            "label": "🎥 Video Prompt"
            if agent_type == "cinematographer"
            else "🎵 Music Style Prompt",
            "max_chars": 500,
            "height": 100,
