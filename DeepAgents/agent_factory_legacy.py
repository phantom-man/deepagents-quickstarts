    ...
    def run_tools(state: AgentState):
        ...
        if (
            ...
            try:
                ...
                input_line = next(
                    (l for l in reversed(lines) if "Action Input:" in l), None
                )
                ...
                if action_line and input_line:
                    ...
                    if action_name in tool_map:
                        ...
                        return {
                            "messages": [
                                HumanMessage(content=f"Observation: {str(result)}")
                            ]
                        }
                    else:
                        return {
                            "messages": [
                                HumanMessage(
                                    content=f"Observation: Error: Tool '{action_name}' not found."
                                )
                            ]
                        }
                ...
                return {
                    "messages": [
                        HumanMessage(content=f"Observation: Error parsing action: {e}")
                    ]
                }

        return {
            "messages": [HumanMessage(content="Observation: Could not parse Action.")]
        }
...
def create_deep_agent(
    ...
    if HAS_DEEPAGENTS_LIB and _lib_create_deep_agent:
        ...
        if not is_replicate:
            ...
            logger.info(
                "Replicate Model detected. Bypassing 'deepagents' lib (native tool binding unsupported). Using Legacy ReAct fallback."
            )

    logger.warning(
        "Falling back to simple create_react_agent (deepagents lib not found OR Replicate model used)"
    )
    ...
    logger.info(
        "Building Manual LangGraph for Robustness... (Attempt 2: Force Tool Config)"
    )
        ...
        try:
            ...
            if "validation error" in str(e):
                ...
                return {
                    "messages": [
                        AIMessage(
                            content=f"System Error: Tool Binding Failed ({e}). Attempting to continue without tools."
                        )
                    ]
                }
            raise e
