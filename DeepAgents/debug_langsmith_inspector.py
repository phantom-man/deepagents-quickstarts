    ...
    try:
        ...
        for i, root_run in enumerate(root_runs):
            ...
            if root_run.end_time and root_run.start_time:
                duration = (
                    f"{(root_run.end_time - root_run.start_time).total_seconds():.2f}s"
                )
            print(f"  Duration: {duration}")

            if root_run.inputs:
                print(
                    f"  📥 Inputs: {str(root_run.inputs)[:200]}..."
                    if len(str(root_run.inputs)) > 200
                    else f"  📥 Inputs: {root_run.inputs}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--limit", type=int, default=5, help="Number of threads to list"
    )
    parser.add_argument(
        "--all", action="store_true", help="Show all run types (not just LLM/Chain)"
    )
